...
class OidcAuthProxyAutoConfiguration {
    ...
    fun jwtDecoder(properties: OidcProperties): JwtDecoder {
        ...
        return when {
            !jwkSetUri.isNullOrBlank() -> NimbusJwtDecoder.withJwkSetUri(jwkSetUri).build()
            !issuerUri.isNullOrBlank() -> NimbusJwtDecoder.withIssuerLocation(issuerUri).build()
    ...
    @Bean
    fun resourceServerCustomizer(jwtAuthProxyConverter: JwtAuthProxyConverter): Customizer<OAuth2ResourceServerConfigurer<HttpSecurity>> =
        Customizer { cfg ->
